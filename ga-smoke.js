...
(async () => {
  ...
  // Determine base URL: prefer CLI arg, then BASE_URL env var, otherwise use file://
  const cliArg = process.argv[2];
  const baseArg = cliArg || process.env.BASE_URL;
  const base = baseArg ? baseArg.replace(/\/$/, '') + '/' : pathToFileURL(process.cwd()).href;

  for (const p of pagesToTest) {
    const resolved = new URL(p.url, base).href;
    console.log('\n--- Loading', resolved);
    try {
